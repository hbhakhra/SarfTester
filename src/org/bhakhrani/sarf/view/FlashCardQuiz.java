...
public class FlashCardQuiz 
{
	...
	private final static String FILE_NAME = "verb_List_Formatted_txt.txt";
	private static FlashCardQuiz INSTANCE = null;
	
	/**
	 * Static factory method for default constructor
	 * @return
	 */
	public static FlashCardQuiz getFlashCardQuiz(InputStream stream) {
		if (INSTANCE == null) {
			...
			tables.add(maadi);
			tables.add(mudaari);
			INSTANCE = new FlashCardQuiz(FILE_NAME, tables, stream);
		}
		return INSTANCE;
	}
	
	public FlashCardQuiz(String fileName, ArrayList<Table> tables, InputStream stream)
	{
		...
		allTables = tables;
		Parser ourParser = new Parser(stream);
